...
function DraggableSequenceCard({ item, isSolved }: { item: SequenceItem; isSolved: boolean }) {
  ...
  return (
    <div
      ...
      {...listeners}
      {...attributes}
      className={`relative w-24 h-32 sm:w-34 sm:h-44 bg-white border-2 border-slate-800 rounded-xl p-1.5 flex flex-col items-center justify-between cursor-grab active:cursor-grabbing shadow-sm hover:shadow-md transition-shadow select-none touch-none ${
        isDragging ? 'opacity-20 shadow-lg border-indigo-400 scale-105' : ''
...
function DroppablePlaceholder({
  ...
  return (
    <div
      ref={setNodeRef}
      className={`relative w-24 h-32 sm:w-34 sm:h-44 rounded-xl border-2 border-dashed flex flex-col items-center justify-center transition-all ${
        placedItem
...
export default function SequenceScreen({
  ...
  useEffect(() => {
    ...
    const jumbled = sortedCopy.sort((a, b) => (a.order === 1 ? -1 : 1));
  ...
  return (
    <DndContext sensors={sensors} onDragStart={handleDragStart} onDragEnd={handleDragEnd}>
      <div className="flex-1 flex flex-col items-center justify-center w-full max-w-4xl mx-auto gap-2.5 sm:gap-5 overflow-hidden px-4">
        {/* Category Header */}
        <span className="text-[10px] sm:text-xs font-black text-amber-600 bg-amber-50 px-3 py-0.5 rounded-full border border-amber-100 uppercase tracking-widest shrink-0">
        ...
        <motion.div
          ...
          className="flex flex-row items-center justify-center gap-3 sm:gap-6 mt-1 w-full shrink-0"
        >
        ...
        <DroppablePoolContainer
          id="jumbled-pool"
          className="h-[105px] sm:h-[140px] w-full bg-slate-50/50 rounded-2xl border-[2px] border-dashed border-slate-350 p-2 sm:p-3 flex items-center justify-center gap-3 sm:gap-6 mt-1 shrink-0 overflow-hidden"
        >
          <AnimatePresence>
