...
function DraggableSequenceCard({ item, isSolved }: { item: SequenceItem; isSolved: boolean }) {
  ...
  return (
    <div
      ...
      {...listeners}
      {...attributes}
      className={`relative w-24 h-32 sm:w-34 sm:h-44 bg-white border-2 border-slate-800 rounded-xl p-1.5 flex flex-col items-center justify-between cursor-grab active:cursor-grabbing shadow-sm hover:shadow-md transition-shadow select-none touch-none ${isDragging ? 'opacity-20 shadow-lg border-indigo-400 scale-105' : ''
